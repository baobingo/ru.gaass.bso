...
class UpcomingShipments extends React.Component {
    ...
    componentDidMount() {
        Client({
            method: 'GET',
            path: '/api/shipments/search/findShipmentByCompletedFalseOrderByPriority'
        }).done(response => {
    ...
    onRowDoubleClick = (index) => {
        this.props.history.push(this.state.upcomingShipments[index]._links.self.href.match(/(^http:\/\/.*:\d*\/api)(.*$)/)[2]);
    }
                        ...
                        {this.state.upcomingShipments.map((row, index) => {
                                ...
                                return (
                                    <TableRowSortable index={index} key={index}
                                                      onDoubleClick={() => this.onRowDoubleClick(index)}>
...
export default withRouter(withStyles(styles)(UpcomingShipments));